...
class DataWorker(object):
    def __init__(self, args, rank):
        ...
        self.model, input_size, self.quant_model = self.initialize_model(args.model_name,
    ...
    def compute_gradients(self, weights):
        ...
        try:
            batch = next(self.dataloaders_dict['train'])
        except StopIteration:  # When the epoch ends, start a new epoch.
            self.dataloaders_dict['train'] = iter(self.dataloaders_dict['train'])
            batch = next(self.dataloaders_dict['train'])
